  ...
  execute(message, args, bot) {
    ...
    if (voiceChannel !== undefined) {
      ...
      if (bot.voiceConnections.size === 1) {
        bot.voiceConnections.get(message.guild.id).playStream(stream, streamOptions);
      } else {
        voiceChannel
          .join()
          .then(connection => {
            ...
            const isStreamOn = () => {
              ...
              if (bot.voiceConnections.size > 0 && !bot.voiceConnections.get(message.guild.id).speaking) {
